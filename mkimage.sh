...
echo "BOARD_AVB_ENABLE is false, make boot.img from kernel."
    mkbootimg --kernel $KERNEL_DEBUG --ramdisk $OUT/ramdisk.img --second $TARGET_PREBUILT_RESOURCE --os_version $PLATFORM_VERSION --header_version $BOARD_BOOTIMG_HEADER_VERSION --os_patch_level $PLATFORM_SECURITY_PATCH --cmdline "$BOARD_KERNEL_CMDLINE" --output $OUT/boot.img && \
    cp -a $OUT/boot.img $IMAGE_PATH/boot.img
fi
echo "done."
...
echo "BOARD_AVB_ENABLE is false, make recovery.img from kernel && out."
    ...
    mkbootimg --kernel $KERNEL_DEBUG --ramdisk $OUT/ramdisk-recovery.img --second $TARGET_PREBUILT_RESOURCE --os_version $PLATFORM_VERSION --header_version $BOARD_BOOTIMG_HEADER_VERSION --recovery_dtbo $BOARD_DTBO_IMG --os_patch_level $PLATFORM_SECURITY_PATCH --cmdline "$ROCKCHIP_RECOVERYIMAGE_CMDLINE_ARGS" --output $OUT/recovery.img && \
    cp -a $OUT/recovery.img $IMAGE_PATH/recovery.img
fi
echo "done."
